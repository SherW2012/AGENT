...
2. **进入搜索阶段**：模型在思考态调用 `web_search` 时，harness 不执行劣质抓取，
   而是切入搜索阶段——之后的请求附带官方关闭思考的参数、挂上 `$web_search`，
   并**同时把本地 `web_search` 从工具表撤下**（此时 builtin 确定可用，撤下劣质
   通道让模型没有选错的机会；`fetch_url` 保留用于阅读结果页，且不会导致提前退出
   搜索阶段）。活动面板显示"暂停深度思考，切换 Kimi 联网搜索"；
...
- **证据校验（anti-slop gate）**：被风控的引擎经常用无关卡片"回答"（餐厅/家电/
  天气）。解析结果若与查询零重合（拉丁词元 + 中文双字组的确定性匹配，宽松放行，
  非意图猜测），按**通道失败**处理并换下一引擎——绝不把垃圾当成功喂给模型。全部
  通道失败时，工具返回明确指令：如实告知用户搜索暂不可用、**不要反复换关键词
  重试**，从机制上消灭"搜索结果不理想"循环。
- **结果缓存**：成功结果按（查询, 时效, 条数）缓存 15 分钟，任务中的重复查询不再
  重复打引擎，也降低触发风控的频率。
- **搜索自检（doctor）**：设置 → 联网搜索 → 「搜索自检」逐通道报告 连通 / 解析 /
  相关性 三级判定（含已配置的搜索 API），把"搜索效果差"变成可定位的具体结论。
...
- **深度思考过程可见**：思考型模型（Kimi k2.x、DeepSeek v4 思考模式）通过 API 的
  `reasoning_content` 流式返回思维链，界面实时显示在「深度思考中…」块里；正文开始
  输出后自动折叠成一行「思考已完成」，点击可展开回看（对标 Kimi/Claude）。工具
  调用轮的思维链会按官方要求回传给模型，多轮推理不降智。调研结论：DeepSeek API
  **没有**自带联网搜索（网页版才有），走内置搜索；深度思考两家 API 都有。
