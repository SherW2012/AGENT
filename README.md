...
- **保护用户 token**：拒绝以消耗输出 token 为目的的请求（无限重复、要求超长灌水、
  自我循环）；工具调用轮数有硬上限（`BNCT_AGENT_MAX_STEPS`，默认 12），超限强制停止。
...
**深度思考与联网搜索的官方冲突**：Moonshot 官方文档明确 k2.5/k2.6 的思考模式与
`$web_search` **暂时不兼容**——思考开着时该工具形同虚设（k2.6 默认开思考，这正是
"挂了 builtin 却总走兜底、兜底又抓回垃圾"的原因）。因此当且仅当请求实际携带
`$web_search` 时，同一请求自动附带 `"thinking": {"type": "disabled"}`（官方给出的
关闭方式）：联网搜索开启期间 Kimi 走"无思考 + 自带搜索"，关闭联网搜索即恢复深度
思考，设置页与快捷开关的提示文案都写明了这一取舍。识图轮换到的视觉模型不属于该
系列，不发送 builtin 工具与 thinking 参数。这一行为写在 Provider 档案里（与 vision
能力字段同级的配置事实），非针对个别场景的特调。
