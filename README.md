...
**搜索来源（推荐配置正规搜索 API）**：Claude Code 的 WebSearch 走的是服务端付费搜索
API，免费抓取的质量天花板远低于此。设置 → 联网搜索 → 搜索来源可选：

| 来源 | 说明 |
|---|---|
| 内置抓取（默认） | 免费、零配置；Bing/DDG 网页抓取，质量有限且可能被风控 |
| 博查 Bocha | 国内直连、中文效果好；bochaai.com 按量付费 |
| Tavily | 面向 Agent 设计；tavily.com 有每月免费额度，需国际网络 |
| Brave | brave.com/search/api 有免费额度，需国际网络 |

选择非内置来源后填入对应 API Key（与模型 Key 一样仅保存在本机进程内；也可用环境变量
`BNCT_AGENT_SEARCH_PROVIDER` / `BNCT_AGENT_SEARCH_API_KEY` 持久化）。API 查询同样整句
送出；API 调用失败自动回退到内置抓取，不会让任务卡死。
